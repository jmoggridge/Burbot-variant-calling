...
In the current work I compared variant calling by two tools: `bcftools mpileup/call` (previously `samtools`) and `freebayes`. Both `bcftools` and `freebayes` apply Bayesian models to estimate genotype posterior probabilities, though they differ in how this is implemented. `bcftools` uses a two step process where `mpileup` first creates the 'pileup' of all reads at variant sites, and then `call` is used to infer genotypes at these [@li2011c; @danecek2021a]. This implementation has the advantage of being very fast. `freebayes` a uses different approach to inferring haplotypes in that it does not use the alignment but the full reads [@garrison2012a; @howtofa].

In this work, `bcftools` and `freebayes` variant callers were used to obtain biallelic SNPs and results were compared in terms of the number of SNPs, the overlap between sets, the depth at each position, and their minor allele frequencies.

Best method could depend on nature of the sequencing data: eg. whole-genome, RAD-seq, RNA-seq, scRNA-seq, etc, and the depth of coverage.
...
Overlap <- read_delim('./data/Overlap.tsv', '\t') %>%  
  distinct() %>% 
  bind_rows(tibble(SNPs = c('mpileup', 'freebayes'), 
                   Count = c(.[1,2][[1]] + .[3,2][[1]], 
                             .[2,2][[1]] + .[3,2][[1]]))) %>%  
  arrange(desc(Count))
```

```{r load_data2, message=FALSE, echo=FALSE}
# Allele frequencies
...
```{r figs, echo=F}
## this chunk contains code for all the plots
library(ggbeeswarm)
library(patchwork)
library(rcartocolor)
library(gridExtra)
...
fig_sitedepth <- 
  ...
  geom_vline(aes(xintercept = mean(allele_depth), color = caller),
             lty = 2) +
  scale_x_log10() +
  labs(x = 'depth per locus\n(sum of 10 individuals)', y = 'density') +
  scale_color_carto_d(palette = 2, direction = -1) +
  scale_fill_carto_d(palette = 2, direction = -1) +
...
# Quality score distribution by caller fig
...
```{r fig1, echo=F, fig.align = 'center', fig.width=8, fig.height=3.5, fig.cap="(A) Distribution of minor allele frequency; SNPs with frequency = 0 or 1 were omitted. (B) Distribution of depth per locus summed across all individuals (note log-scaled x-axis)."}
...
```{r fig_allele_maf_nchr, echo=F, fig.align='center', fig.height=4, fig.cap="Allele depth and minor allele frequency do not show any correlation; minor allele frequency is constrained to specific values by the number of chromosomes represented"}
fig_alleledepth_over_maf
```

## Mean depth by subject

Mean depth for each individual across all loci was computed as follows:
...
We can see the all individuals have greater mean depth at SNPs called by `freebayes` than `bcftools mpileup` (fig. 4)

```{r fig_indv_depth, echo=F, fig.align = 'center', fig.width=5, fig.height=2.9, fig.cap="Mean depth per locus for each individual is greater for SNPs called by  freebayes than bcftools."}
fig_indvdepth
```

### Quality scores

I obtained quality scores for each loci from bctools stats

```{r engine = 'bash', eval=FALSE}
## quality scores...
bcftools stats mpileup.filter.recode.vcf > mpileup.filter.recode.stats
grep -E "Quality" mpileup.filter.recode.stats | cut -f1 --complement > mpileup.quality.stats
grep ^QUAL mpileup.filter.recode.stats | cut -f1 --complement >> mpileup.quality.stats bcftools stats freebayes.filter.recode.vcf > freebayes.filter.recode.stats
...
```{r fig3, echo=F, fig.align = 'center', fig.width=5, fig.height=2.9, fig.cap="Minor allele fre"}
fig_alleledepth_over_maf
```

```{r fig4, echo=F, fig.height = 3, fig.align = 'center', fig.width=5, fig.height=2.9, fig.cap="The quality score distributions of freebayes and bcftools mpileup "}
fig_quality_dist
```
